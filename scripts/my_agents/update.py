...
Scan ONLY the subsequent filings for settled events that change the balance sheet (e.g. stock issuances, debt repayments, acquisitions).  Ignore pending or proposed transactions.
...
Applying this change caused the balance sheet to be unbalanced. Double-check the referenced filings via FileSearchTool and return a corrected FilingChange whose deltas will balance the sheet. If no corrected values can be found, return the original FilingChange unchanged.
